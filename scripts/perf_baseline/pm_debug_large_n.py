"""Shape-by-shape LARGE_N debug: PM top-1 vs PM oracle (best in PM's ranked set)
vs autotune. Ranking-gap (top1<<oracle) vs candidate-gap (oracle<<autotune)."""
...
SHAPES=[(M,N,K) for N,K in [(24576,1536),(32768,512)] for M in [4,8,16,32,64,128,256]]
...
print(f"{'shape':>16}{'PMtop1':>8}{'oracle':>8}{'auto':>7}{'t1/au':>7}{'or/au':>7}  PM_cfg -> oracle_cfg(rank)")
for (M,N,K) in SHAPES:
    ...
    print(f"{f'{M}x{N}x{K}':>16}{T1:>8.0f}{BO:>8.0f}{TA:>7.0f}{T1/TA if TA else 0:>7.2f}{BO/TA if TA else 0:>7.2f}  {cfgstr(pmc)} -> {cfgstr(boc)}(#{bor})")
