...
struct Utils {
  ...
  static InstClass classifyInstruction(const Instruction &I) {
    ...
    if (isa<UncondBrInst>(I) || isa<CondBrInst>(I))
...
private:
  ...
  static void moveAnchors(SmallVectorImpl<AnchorInst> &Anchors,
    ...
    // Build a position map for instructions in [LastLW, FirstGR) so we can
    // (a) check whether an arbitrary user is inside the move-source range, and
    // (b) sort the final ToMove list back into original program order before
    // emitting moveAfter calls. The latter is what keeps SSA dominance valid
    // when transitive users (e.g. icmp → select → ...) of an LR's value are
    // pulled along with the LR.
    DenseMap<Instruction *, unsigned> rangePos;
    {
      unsigned idx = 0;
      bool inR = false;
      for (Instruction &I : Utils::instructionsInRegion(Region)) {
        if (&I == LastLW) { inR = true; continue; }
        if (&I == FirstGR) break;
        if (!inR) continue;
        rangePos[&I] = idx++;
      }
    ...
    // Transitively collect users of moved LR results that themselves live
    // inside the move-source range. Without this closure, a chain like
    //   %x = ds_read_tr (LR)              ← moved
    //   %y = icmp eq %x, 0                ← immediate user, also moved
    //   %z = select i1 %y, ...            ← transitive user, NOT moved (bug!)
    // ends up with %z (still in original spot) referring to %y at its new
    // post-move position — i.e. a use that no longer dominates. The verifier
    // catches this as "Instruction does not dominate all uses".
    SmallVector<Instruction *, 16> userWorklist;
    for (Instruction *I : ToMove) {
      if (Utils::classifySchedInst(*I) == SchedKind::LR)
        userWorklist.push_back(I);
    }
    while (!userWorklist.empty()) {
      Instruction *I = userWorklist.pop_back_val();
      for (User *U : I->users()) {
        auto *UI = dyn_cast<Instruction>(U);
        if (!UI) continue;
        if (UI->getParent() != Region.BB) continue;
        if (!rangePos.count(UI)) continue;          // outside [LastLW, FirstGR)
        if (!ToMoveSet.insert(UI).second) continue; // already collected
        ToMove.push_back(UI);
        userWorklist.push_back(UI);                 // transitive
      }
    }
    ...
    // Sort ToMove by original program-order position so chained moveAfter
    // (which appends each moved instruction immediately after the prior one)
    // reproduces the original relative order — defs before uses.
    std::sort(ToMove.begin(), ToMove.end(),
              [&](Instruction *a, Instruction *b) {
                auto pa = rangePos.find(a);
                auto pb = rangePos.find(b);
                unsigned ia = (pa != rangePos.end()) ? pa->second : ~0u;
                unsigned ib = (pb != rangePos.end()) ? pb->second : ~0u;
                return ia < ib;
              });
    ...
    LLVM_DEBUG(dbgs() << "  Moving " << ToMove.size()
                      << " instructions (LR/waitcnt/barrier + transitive users)"
                      << " after 2nd-to-last GR\n");
  ...
  static void hoistVoffsetCompute(BasicBlock &BB) {
    ...
    while (!Worklist.empty()) {
      ...
      bool Simple =
          ...
          // ICmp/FCmp must be allowed because Select takes an i1 condition
          // typically produced by ICmp; without these, Select gets hoisted
          // while its ICmp operand stays put, breaking dominance.
