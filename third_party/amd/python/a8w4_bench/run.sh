...
echo "== BASELINE DB SWEEP START $(date '+%F %H:%M:%S')  (112 runs: JSON + PM-canonical)"
cleanup_container
for shape in "${SHAPES[@]}"; do
  read -r ISL OSL MML <<< "$shape"
  for TP in "${TPS[@]}"; do
    for CONC in "${CONCS[@]}"; do
      run_arm json    "$TP" "$CONC" "$ISL" "$OSL" "$MML"
      run_arm pmcanon "$TP" "$CONC" "$ISL" "$OSL" "$MML"
    done
  done
done
cleanup_container
